...
try {
    $testElf = Join-Path $ProjectRoot 'tests\app_telemetry_test.elf'
    $arguments = @(
        ...
        'tests/app_telemetry_test.c', 'user/app/app_telemetry.c', '-lc', '-o', $testElf
    )
    & $Gcc @arguments
    if ($LASTEXITCODE -ne 0) { throw 'App telemetry test compilation failed' }
    python -B tests/run_q5_test.py $testElf --deps (Join-Path $BaselineRoot 'build/testdeps')
    if ($LASTEXITCODE -ne 0) { throw 'App telemetry regressions failed' }
