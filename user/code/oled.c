...
static uint8_t oled_i2c_wait(uint32_t status)
{
    uint32_t timeout = OLED_I2C_TIMEOUT;

    while (timeout-- > 0U)
    {
        if ((DL_I2C_getControllerStatus(OLED_INST) & status) != 0U)
        {
            return 1U;
        }
    }
...
void OLED_WR_Byte(uint8_t dat, uint8_t mode)
{
    ...
    // 1. 等待 I2C 彻底空闲
    if (!oled_i2c_wait(DL_I2C_CONTROLLER_STATUS_IDLE))
    {
        return;
    }
    ...
    (void)oled_i2c_wait(DL_I2C_CONTROLLER_STATUS_IDLE);
}
