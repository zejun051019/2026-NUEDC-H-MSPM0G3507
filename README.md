...
```text
底盘：灰度 / 编码器 → 路线状态机与速度闭环 → 底盘驱动

视觉：视觉模块 → UART3 新帧接收与有效性门控 → 球位置/速度估计
                                              ├─ 第 3 问控制流程
                                              └─ 第 4/5/6 问共用车辆控制状态机
                                                   ├─ Q6 目标捕获 / 显式目标
                                                   ├─ Q5/Q6 运动状态前馈
...
main.c 前台协作调度：输入与控制任务 → 状态/遥测发布 → OLED 更新
TIMG6 / UART / GPIO 中断：时基、收发和事件采集；格式化日志留在前台
```

上图概括源码模块关系，不是精确实时周期图。具体任务调用顺序、控制边界与验证限制见[架构说明](docs/architecture.md)、[控制流程](docs/control-algorithm.md)和[测试说明](docs/testing.md)。
